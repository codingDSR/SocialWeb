...
*/
		$sql = "SELECT COUNT(id) FROM friends WHERE user1_id='$log_id' AND user2_id='$user' AND accepted='1' LIMIT 1";
		$query = mysqli_query($db_conx, $sql);
		$row_count1 = mysqli_fetch_row($query);
		$sql = "SELECT COUNT(id) FROM friends WHERE user1_id='$user' AND user2_id='$log_id' AND accepted='1' LIMIT 1";
		$query = mysqli_query($db_conx, $sql);
		$row_count2 = mysqli_fetch_row($query);
		$sql = "SELECT COUNT(id) FROM friends WHERE user1_id='$log_id' AND user2_id='$user' AND accepted='0' LIMIT 1";
		$query = mysqli_query($db_conx, $sql);
		$row_count3 = mysqli_fetch_row($query);
		$sql = "SELECT COUNT(id) FROM friends WHERE user1_id='$user' AND user2_id='$log_id' AND accepted='0' LIMIT 1";
		$query = mysqli_query($db_conx, $sql);
...
*/
        if ($row_count1[0] > 0 || $row_count2[0] > 0) {
	        ...
	        $sql = "INSERT INTO friends(user1_id, user2_id, datemade) VALUES('$log_id','$user',now())";
		    $query = mysqli_query($db_conx, $sql);
			mysqli_close($db_conx);
		...
		$row_count2 = mysqli_fetch_row($query);
	    if ($row_count1[0] > 0) {
	        $sql = "DELETE FROM friends WHERE user1_id='$log_id' AND user2_id='$user' AND accepted='1' LIMIT 1";
			$query = mysqli_query($db_conx, $sql);
			mysqli_close($db_conx);
	        echo "unfriend_ok";
	        exit();
	    } else if ($row_count2[0] > 0) {
			$sql = "DELETE FROM friends WHERE user1_id='$user' AND user2_id='$log_id' AND accepted='1' LIMIT 1";
			$query = mysqli_query($db_conx, $sql);
			mysqli_close($db_conx);
	        echo "unfriend_ok";
	        exit();
...
if (isset($_POST['action']) && isset($_POST['reqid']) && isset($_POST['user1'])){
	...
	if($_POST['action'] == "accept"){
	    ...
	    if ($row_count1[0] > 0 || $row_count2[0] > 0) {
			...
			$sql = "UPDATE friends SET accepted='1' WHERE id='$reqid' AND user1_id='$user' AND user2_id='$log_id' LIMIT 1";
			$query = mysqli_query($db_conx, $sql);
			mysqli_close($db_conx);
	        echo "accept_ok";
	        exit();
		...
		mysqli_query($db_conx, "DELETE FROM friends WHERE id='$reqid' AND user1_id='$user' AND user2_id='$log_id' AND accepted='0' LIMIT 1");
		mysqli_close($db_conx);
		echo "reject_ok";
		exit();
	}
